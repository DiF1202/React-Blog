import React, { memo, useState, createElement, useCallback, useRef } from 'react';
import { Modal, Input, Comment, Tooltip, Avatar, message } from 'antd';
import { CommentItemWrap } from './style';
import { getArticleCommentListAction, changeArticleCommentListAction } from '@/redux/reducers/detail/actionCreators';
import { LikeOutlined, LikeFilled, DeleteOutlined } from '@ant-design/icons';
import { useDispatch } from 'react-redux';
import { addComment } from '@/network/detail';
import { SelfSelector } from '@/utils/common';
import { changeMainLoadingAction } from '../../../Layout/store/actionCreators';
...
const CommentItem = props => {
  ...
  const actions = [
    ...
    <span key="comment-basic-reply-to" onClick={() => deleteComent()} style={{ color: '#1890FF' }}>
      删除
      <DeleteOutlined />
    </span>
  ];
  ...
  return (
    <CommentItemWrap ref={CommentRef} isShow={isShow} className={['shy-comment', `commentItem${index}`].join(' ')}>
      <Comment
        ...
        avatar={<Avatar className="avatar" src={item.qqurl} alt={item.username} />}
        content={
          <>
            {item.fathername && <span className="comment_father">@{item.fathername}:</span>}
            <div className="markdown-body" dangerouslySetInnerHTML={{ __html: item.comment }}></div>
          </>
        }
        datetime={
      ...
      />
      <Modal title="输入您想说的话~" visible={visible} onOk={() => hideModal(1)} onCancel={() => hideModal(2)} okText="确认" cancelText="取消">
        <Input placeholder={`回复@${item.username}`} style={{ borderRadius: '5px' }} disabled={true} />
        <TextArea value={comment} rows={4} onChange={e => CommentChange(e)} style={{ marginTop: '10px', borderRadius: '5px' }} />
        <div>
          {' '}
          <span style={{ color: '#ec5328' }}>(tip:支持markdown语法)</span>
        </div>
      </Modal>
    </CommentItemWrap>
  );
