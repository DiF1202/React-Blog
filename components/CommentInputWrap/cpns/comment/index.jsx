    ...
    <CommentWrap>
      {commentList &&
        commentList.map((item, index) => {
          return (
            <div key={item.id}>
              <CommentItem type3={type3} index={index} isShow={isShowArray[index]} levelId={item.id} item={item} article_id={article_id}></CommentItem>
              <div className="children">
                {item.children.map(item2 => {
                  return <CommentItem isShow={isShowArray[index]} type3={type3} article_id={article_id} levelId={item.id} item={item2} key={item2.id}></CommentItem>;
                })}
              </div>
            </div>
          );
