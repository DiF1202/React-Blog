    ...
    <CommentInputWraper>
      <div className="comment_input_wrap">
        <div className="input_and_submit" style={{ textAlign: 'right' }}>
          {articleDetail && (
            <div className="hdf_tip">
              可以在这里发表您的看法或则建议<span style={{ color: '#ec5328' }}>(支持markdown语法)</span>
            </div>
          )}
          <TextArea
            style={{
            ...
            value={comment}
          />
          <Button onClick={submitComment} style={{ marginTop: '15px' }} type="primary">
            提交评论
          </Button>
        </div>
        {!articleDetail || Number(articleDetail.openComment) === 1 ? (
          <div>
            <Comment commentList={commentList} article_id={article_id} type3={type3}></Comment>
            <p
              style={{
                textAlign: 'center',
                color: '#1890FF ',
