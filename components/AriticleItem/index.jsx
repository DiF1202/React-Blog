...
import { ScheduleOutlined, MessageOutlined, FireOutlined, ToTopOutlined } from '@ant-design/icons';
...
const ArticleItem = props => {
  ...
  const { item, index, homeFontColor, isShow } = props;

  return (
    <HomeArticleItem ref={ArticleRef} homeFontColor={homeFontColor} isShow={isShow} className={['articeItem', `homeItem${index}`].join(' ')}>
      {/* 下面这一行是右上角的实在小图标 */}
      ...
      >
        ...
        <div className="article_info">
          ...
          {item?.tags?.map(tag => {
            return (
              <span key={tag.tag_id} className="tag_item" style={{ backgroundColor: tag.tag_color }}>
                {tag.tag_name}
              </span>
            );
          })}
          ...
          <div className="readingCount">
            <FireOutlined style={{ fontSize: '16px', color: 'red' }} /> {item.readingCount}
          </div>
          {/* 评论数 */}
